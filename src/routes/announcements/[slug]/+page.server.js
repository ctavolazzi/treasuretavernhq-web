...
// For this example, we're using the hard-coded announcements data as fallback
const announcements = [
  {
    id: 1,
...
export async function load({ params }) {
  try {
    // First try to load announcement from Markdown file
    const mdFilePath = path.join(contentDir, `${params.slug}.md`);

    // Check if the Markdown file exists
    let announcement;
    let usesMarkdown = false;
    ...
    try {
      ...
      // Create announcement object from Markdown
      announcement = {
        slug: params.slug,
        ...
        relatedAnnouncements: frontmatter.relatedAnnouncements || []
      };
    } catch (err) {
      // If Markdown file doesn't exist or can't be read, use hardcoded data
      usesMarkdown = false;
      announcement = announcements.find(a => a.slug === params.slug);
    ...
    let relatedAnnouncements = [];
    if (announcement.relatedAnnouncements && announcement.relatedAnnouncements.length > 0) {
      relatedAnnouncements = announcement.relatedAnnouncements.map(id => {
        // For Markdown based relatedAnnouncements, we might have slugs instead of IDs
        if (typeof id === 'string') {
          // Try to find by slug
          return announcements.find(a => a.slug === id);
        } else {
          // Find by id
    ...
    throw error(500, "Error loading announcement");
  }
}